...
import {
  ...
  Copy, Package, Sparkles, ArrowLeft, RotateCcw, Phone, Store,
} from "lucide-react";

export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      { title: "Painel Administrativo — Catálogo Inteligente" },
      ...
      <main className="mx-auto max-w-6xl px-4 pt-6">
        <Tabs defaultValue="dashboard" className="space-y-6">
          ...
          <TabsContent value="dashboard">
            <DashboardTab templates={templates} resellers={resellers} />
          </TabsContent>
          <TabsContent value="templates">
            <TemplatesTab templates={templates} />
          </TabsContent>
          <TabsContent value="resellers">
            <ResellersTab resellers={resellers} templates={templates} />
          </TabsContent>
        </Tabs>
      </main>
    </div>
  ...
  return (
    <div className="space-y-6">
        ...
        {stats.map((s) => (
          <Card key={s.label} className="rounded-2xl p-5 shadow-soft border-border/60">
            <div className="flex items-center gap-3">
              <span className="w-10 h-10 rounded-xl bg-secondary grid place-items-center text-primary">
      ...
      <Card className="rounded-2xl p-5 shadow-soft border-border/60">
        ...
        {resellers.length === 0 ? (
          <p className="text-sm text-muted-foreground">Nenhum revendedor cadastrado.</p>
        ) : (
            ...
            {resellers.slice(0, 5).map((r) => (
              <div key={r.slug} className="flex items-center justify-between py-2 border-b border-border/40 last:border-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-medium">{r.storeName}</div>
                    <div className="text-xs text-muted-foreground">/loja/{r.slug}</div>
                  </div>
                </div>
                <Button asChild size="sm" variant="ghost" className="rounded-full">
                  <Link to="/loja/$slug" params={{ slug: r.slug }} target="_blank">
...
function TemplatesTab({ templates }: { templates: CatalogTemplate[] }) {
  ...
  function startNew() {
    setEditing({
      id: "", name: "", description: "", banner: "", products: [], createdAt: 0,
    });
    setOpen(true);
  }
  function startEdit(t: CatalogTemplate) {
    setEditing({ ...t, products: t.products.map((p) => ({ ...p })) });
    setOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {templates.length === 0 ? (
        <Card className="rounded-2xl p-10 text-center text-muted-foreground shadow-soft">
            ...
            <Card key={t.id} className="rounded-2xl p-5 shadow-soft border-border/60 flex flex-col">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{t.description || "Sem descrição"}</p>
                </div>
              ...
              <div className="mt-auto pt-4 flex gap-2">
                <Button size="sm" variant="outline" className="rounded-full flex-1" onClick={() => startEdit(t)}>
                  <Pencil className="w-3 h-3 mr-1" /> Editar
                </Button>
                <Button
                  size="sm" variant="ghost" className="rounded-full text-destructive"
                  onClick={() => { if (confirm(`Excluir modelo "${t.name}"?`)) adminStore.deleteTemplate(t.id); }}
                >
...
function TemplateDialog({
  open, onOpenChange, editing, setEditing,
}: {
  open: boolean;
  onOpenChange: (v: boolean) => void;
  editing: CatalogTemplate | null;
  setEditing: (v: CatalogTemplate | null) => void;
}) {
  ...
  function save() {
    if (!editing) return;
    if (isNew) {
      adminStore.createTemplate({ name: editing.name, description: editing.description, banner: editing.banner, products: editing.products });
    } else {
      adminStore.updateTemplate(editing.id, { name: editing.name, description: editing.description, banner: editing.banner, products: editing.products });
    }
    onOpenChange(false);
    setEditing(null);
  }
  ...
  function addProduct() {
    ...
    setEditingProduct({ id: `p_${Date.now()}`, name: "", description: "", price: 0, image: "", category: cats[0] || "Perfumes" });
    setProductOpen(true);
  }
  ...
  function saveProduct() {
    ...
    const p: Product = {
      id: editingProduct.id, name: editingProduct.name?.trim() || "Sem nome",
      description: editingProduct.description?.trim() || "", price: Number(editingProduct.price) || 0,
      oldPrice: editingProduct.oldPrice ? Number(editingProduct.oldPrice) : undefined,
      image: editingProduct.image?.trim() || "", category: editingProduct.category || "Perfumes",
  ...
  function removeProduct(id: string) {
    if (!editing) return;
    setEditing({ ...editing, products: editing.products.filter((p) => p.id !== id) });
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display">{isNew ? "Novo Modelo" : "Editar Modelo"}</DialogTitle>
        </DialogHeader>
        {editing && (
          <div className="space-y-4">
            <Field label="Nome do Modelo">
              <Input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} placeholder="Ex: Catálogo Premium" />
            </Field>
            <Field label="Descrição">
              <Textarea value={editing.description} onChange={(e) => setEditing({ ...editing, description: e.target.value })} rows={2} placeholder="Descrição do modelo..." />
            </Field>
            <Field label="Banner padrão (URL)">
            ...
            <div className="border-t border-border/60 pt-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button size="sm" className="rounded-full bg-primary-gradient" onClick={addProduct}>
                  <Plus className="w-4 h-4 mr-1" /> Produto
                </Button>
              </div>
              {editing.products.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">Nenhum produto. Adicione o primeiro.</p>
              ) : (
                  ...
                  {editing.products.map((p) => (
                    <div key={p.id} className="flex items-center gap-3 py-2 px-3 rounded-xl bg-secondary/50">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">{p.category} · {formatBRL(p.price)}</div>
                      </div>
            ...
            {editingProduct && (
              <div className="space-y-3">
                ...
                <Field label="Categoria">
                  <Input value={editingProduct.category ?? ""} onChange={(e) => setEditingProduct({ ...editingProduct, category: e.target.value })} placeholder="Perfumes, Linha Corporal..." list="cat-suggestions" />
                ...
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={!!editingProduct.featured} onChange={(e) => setEditingProduct({ ...editingProduct, featured: e.target.checked })} /> Destacar
                </label>
              </div>
            )}
            <DialogFooter>
              <Button variant="ghost" onClick={() => setProductOpen(false)}>Cancelar</Button>
              <Button onClick={saveProduct} className="bg-primary-gradient">Salvar</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </DialogContent>
...
function ResellersTab({ resellers, templates }: { resellers: AdminReseller[]; templates: CatalogTemplate[] }) {
  ...
  function startNew() {
    setEditing({ name: "", storeName: "", bio: "", whatsapp: "", instagram: "", templateId: templates[0]?.id || "", banner: "" });
    setIsNew(true);
    setOpen(true);
  }
  function startEdit(r: AdminReseller) {
    setEditing({ ...r });
    setIsNew(false);
    setOpen(true);
  }
  ...
  function save() {
    if (!editing) return;
    if (isNew) {
      adminStore.createReseller({
        name: editing.name || "Revendedor",
        storeName: editing.storeName || editing.name || "Loja",
        bio: editing.bio || "",
      ...
      adminStore.updateReseller(editing.slug, {
        name: editing.name,
  ...
  return (
    <div className="space-y-4">
      ...
      {resellers.length === 0 ? (
        <Card className="rounded-2xl p-10 text-center text-muted-foreground shadow-soft">
          Nenhum revendedor. Crie um modelo primeiro, depois adicione revendedores.
        </Card>
      ) : (
        <div className="space-y-3">
          {resellers.map((r) => {
            ...
            return (
              <Card key={r.slug} className="rounded-2xl p-4 shadow-soft border-border/60">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-primary-gradient grid place-items-center text-primary-foreground font-bold font-display shrink-0">
                    {r.avatarInitials}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-col sm:flex-row gap-1.5 shrink-0">
                    <Button asChild size="sm" variant="outline" className="rounded-full">
                      <Link to="/loja/$slug" params={{ slug: r.slug }} target="_blank">
                        <Store className="w-3.5 h-3.5 mr-1" /> Loja
                      </Link>
                    </Button>
                    <Button asChild size="sm" variant="outline" className="rounded-full">
                      <Link to="/painel/$slug" params={{ slug: r.slug }} target="_blank">
                        <ExternalLink className="w-3.5 h-3.5 mr-1" /> Painel
                      </Link>
                    </Button>
                    <Button size="sm" variant="outline" className="rounded-full" onClick={() => startEdit(r)}>
                      <Pencil className="w-3.5 h-3.5" />
                    </Button>
                    {tpl && (
                      <Button
                        size="sm" variant="outline" className="rounded-full"
                        title="Replicar modelo"
                        onClick={() => {
                          if (confirm(`Replicar modelo "${tpl.name}" para ${r.storeName}? Os preços personalizados serão mantidos.`))
                            adminStore.applyTemplate(r.slug, r.templateId);
                        }}
                      >
                    ...
                    <Button
                      size="sm" variant="ghost" className="rounded-full text-destructive"
                      onClick={() => { if (confirm(`Excluir "${r.storeName}"?`)) adminStore.deleteReseller(r.slug); }}
                    >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-display">{isNew ? "Novo Revendedor" : "Editar Revendedor"}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="space-y-3">
              ...
              <Field label="Banner (URL)"><Input value={editing.banner ?? ""} onChange={(e) => setEditing({ ...editing, banner: e.target.value })} placeholder="https://..." /></Field>

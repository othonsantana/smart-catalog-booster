...
/**
 * Resolve the base reseller data.
 * Checks admin-store first, then falls back to hardcoded RESELLERS.
 */
function getBaseReseller(slug: string): Reseller | undefined {
  const key = slug.toLowerCase();

  // Check admin store (client-side only)
  if (typeof window !== "undefined") {
    ...
    if (adminReseller) {
      ...
      return {
        ...
        banner: adminReseller.banner || DEFAULT_BANNER,
        products,
      };
    }
  }
...
export function mergeReseller(slug: string): Reseller | undefined {
  ...
  // Merge products: if override has products, use those; otherwise use base
  const baseProducts = o.products ?? base.products;

  // Apply price overrides from the reseller panel
...
export function useReseller(slug: string): Reseller | undefined {
  ...
  // Also subscribe to admin store changes
  useSyncExternalStore(

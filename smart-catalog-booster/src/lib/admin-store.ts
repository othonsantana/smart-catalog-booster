...
export type AdminReseller = {
  ...
  banner: string;
  templateId: string;
  products: Product[]; // copied from template at creation/replication
  createdAt: number;
};
...
function seed() {
  // We import images as module paths — use placeholder URLs for seed
  // The actual images are bundled assets, so we reference them via import paths
  ...
  const template: CatalogTemplate = {
    id: templateId,
    name: "Modelo Padrão",
    description: "Template padrão com perfumes, linha corporal e capilar.",
    banner: "",
  ...
  const joana: AdminReseller = {
    ...
    banner: "",
    templateId,
    products: seedProducts.map((p) => ({ ...p, id: p.id + "_j" })),
    createdAt: Date.now(),
  };

  state = {
    templates: { [templateId]: template },
    resellers: { maria: maria, joana: joana },
  };
  persist();
}
...
export const adminStore = {
  ...
  getAllTemplates(): CatalogTemplate[] {
    return Object.values(state.templates).sort((a, b) => b.createdAt - a.createdAt);
  },
  getTemplate(id: string): CatalogTemplate | undefined {
  ...
  updateTemplate(id: string, patch: Partial<Omit<CatalogTemplate, "id" | "createdAt">>) {
    ...
    state = {
      ...state,
      templates: { ...state.templates, [id]: { ...existing, ...patch } },
    };
    emit();
  },
  ...
  getAllResellers(): AdminReseller[] {
    return Object.values(state.resellers).sort((a, b) => b.createdAt - a.createdAt);
  },
  getReseller(slug: string): AdminReseller | undefined {
    return state.resellers[slug.toLowerCase()];
  },
  createReseller(data: {
    name: string;
    storeName: string;
    ...
    const products = template
      ? template.products.map((p) => ({ ...p, id: `${p.id}_${slug}` }))
      : [];
    ...
    const reseller: AdminReseller = {
      ...
      avatarInitials: generateInitials(data.name),
      banner: data.banner || template?.banner || "",
  ...
  updateReseller(slug: string, patch: Partial<Omit<AdminReseller, "slug" | "createdAt">>) {
    const existing = state.resellers[slug];
    if (!existing) return;
    state = {
      ...state,
      resellers: { ...state.resellers, [slug]: { ...existing, ...patch } },
    };
    emit();
  },
  deleteReseller(slug: string) {
    const next = { ...state.resellers };
    delete next[slug];
    state = { ...state, resellers: next };
    emit();
  },

  /**
   * Re-apply a template to a reseller.
   * Copies template products to the reseller, preserving any price overrides
   * the reseller may have set (matched by product name).
   */
  applyTemplate(slug: string, templateId: string) {
    ...
    // Build a map of existing price overrides by product name
    const priceOverrides = new Map<string, { price: number; oldPrice?: number }>();
    for (const p of reseller.products) {
      priceOverrides.set(p.name, { price: p.price, oldPrice: p.oldPrice });
    }

    // Copy products from template, keeping price overrides where names match
    const newProducts = template.products.map((p) => {
      ...
      return {
        ...
        oldPrice: override?.oldPrice ?? p.oldPrice,
      };
    });
    ...
    state = {
      ...
      resellers: {
        ...
        [slug]: {
          ...
          banner: template.banner || reseller.banner,
        },
      },
    };
    emit();
  ...
  /** Full reset: clears everything and re-seeds */
  resetAll() {
    state = emptyState();
    seed();
    emit();

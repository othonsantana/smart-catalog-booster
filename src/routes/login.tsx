...
import { Sparkles, LogIn, AlertCircle, User, ArrowLeft } from "lucide-react";
import { login, getAuth } from "@/lib/auth-store";

export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Entrar — Catálogo Inteligente" },
      { name: "description", content: "Acesse sua conta de revendedor ou administrador no Catálogo Inteligente." },
...
function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-background px-4">
      ...
      <Link to="/" className="absolute top-6 left-6 inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors z-10">
        <ArrowLeft className="w-4 h-4" /> Voltar
      </Link>

      <Card className="relative w-full max-w-sm rounded-3xl shadow-elevated border-border/60 p-8 backdrop-blur-sm bg-card/90">
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          Ainda não tem conta? Fale com o administrador da sua equipe.
        </p>
      </Card>
    </div>
  );
